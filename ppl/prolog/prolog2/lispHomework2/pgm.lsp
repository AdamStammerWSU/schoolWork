...
(defun evalexp (expr tset)
    (cond ((NULL expr) 'F)
        ((equal (car (fillTruthVals expr tset)) 'and) (myAND (cdr (fillTruthVals expr tset)) tset))
        ((equal (car (fillTruthVals expr tset)) 'or) (myOR (cdr (fillTruthVals expr tset)) tset))
        ((equal (car (fillTruthVals expr tset)) 'not) (myNOT (cdr (fillTruthVals expr tset)) tset))
        (T (car (fillTruthVals expr tset)))
    )

)

(defun getTruthVal (term tset)
    (cond ((NULL expr) 'F)
        ((equal term (car (car tset))) (cdr (car tset)))
        (T (getTruthVal term (cdr tset)))
    )
)

(defun fillTruthVals (expr tset)
    (cond   ((NULL tset) expr)
            (T (fillTruthVals (subst (getTruthVal (car (car tset)) tset) (car (car tset)) expr) (cdr tset)))
    )
)


(defun myAND (expr tset)
    (cond   ((null expr) 'T)
            ;just one false will return false
            ((equal (evalexp (car expr) tset) 'F) 'F)
            ; if it's true keep checking for falses
            ((equal (evalexp (car expr) tset) 'T) (myAND (cdr expr) tset))
    )
)

(defun myOR (expr tset)
    (cond   ((null expr) 'F)
            ;just one false will return false
            ((equal (evalexp (car expr) tset) 'T) 'T)
            ; if it's true keep checking for falses
            ((equal (evalexp (car expr) tset) 'F) (myOR (cdr expr) tset))
    )
)


(defun myNOT (expr tset)
    (cond   ((null expr) 'F)
            ;just one false will return false
            ((equal (evalexp (car expr) tset) 'F) 'T)
            ; if it's true keep checking for falses
            ((equal (evalexp (car expr) tset) 'T) 'F)
    )
)