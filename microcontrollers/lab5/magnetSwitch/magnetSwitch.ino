...
void loop()
{
  ...
  if ((lastPrint + PRINT_SPEED) < millis())
  {
    ...
    float ax = imu.calcMag(imu.mx);
    float ay = imu.calcMag(imu.my);
    float az = imu.calcMag(imu.mz);

    bool magnetPresent = false;
    bool orientation1 = false;
    bool orientation2 = false;
    
    if(abs(az) > 1) { // magnet above/below
      magnetPresent = true;    
    } else magnetPresent = false;
    ...
    if((millis() - lastMessageTime) > 1500 && message == lastMessage && lastPrintedMessage != message) {
