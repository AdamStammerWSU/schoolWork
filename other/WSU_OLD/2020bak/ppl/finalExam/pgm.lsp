...
;	(cond 	((null mylist) '(() ()))
;			((= 1 (length mylist)) (list (car mylist) '()))
;			(t (let ((listPair (listPair (rest (rest l)))))
;			(list (list* (first l) (first listPair))
;					(list* (second mylist) (second listPair)))))
;	)
;)
...
;			(let 
;			(	
;				(l (listPair (cdr (cdr myList)))))
 ;           
	;			(list
;					(cons (car myList) (car l))
;					(cons (car (cdr myList)) (car (cdr l)))
;				)
;			)
...
;		(if (cdr (cdr myList)) ; if the list is length 2 or more
;				(list
;					(cons (car myList) (car (listPair (cdr (cdr myList))))) ; combine the first item with the rest of the odd items
;					(cons (car (cdr myList)) (car (cdr (listPair (cdr (cdr myList)))))) ; combine the second ite mwith rest of the even items
;				)
;			
;			(list (car myList) (cdr myList)) ; return the first and second items split
;		)
...
(defun listPair (myList)
	(if myList ; if the list isn't empty 
		(if (cdr (cdr myList)) ; if the list is length 2 or more
			...
			(list (car myList) (cdr myList)) ; return the first and second items split
			;i think this line is causing the extra period but I'm not sure
		)
		'(() ()) ; if the list is empty (base case)
	)
)
